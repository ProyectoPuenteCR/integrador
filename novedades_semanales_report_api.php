...
function ns_rep_snapshot(array $items,$titleColumn=''){
    $snapshot=[];
    ...
    $snapshot[]=['_report_meta'=>'clear-report-v1','title_column'=>(string)$titleColumn];
...
if($action==='batch'){
    ...
    ns_rep_out(['ok'=>true,'count'=>$count,'processed'=>$processed,'message'=>$processed.' elemento'.($processed===1?'':'s').' '.($active?'agregado'.($processed===1?'':'s').' al reporte.':'quitado'.($processed===1?'':'s').' del reporte.')]);
}
...
if($action==='load_saved'){
    ...
    $titleColumn=ns_report_saved_title_column($snapshot);
    ...
    ns_rep_out(['ok'=>true,'count'=>$loaded,'saved_id'=>(int)ns_value($saved,'ID'),'name'=>(string)ns_value($saved,'NOMBRE'),'emailto'=>(string)ns_value($saved,'DESTINATARIOS'),'subject'=>(string)ns_value($saved,'ASUNTO'),'body'=>(string)ns_value($saved,'CUERPO'),'title_column'=>$titleColumn]);
}
if($action==='save_report'){
    ...
    $titleColumn=ns_report_title_column_for_items($input['title_column']??'',$items);
    $snapshot=ns_rep_snapshot($items,$titleColumn);if($snapshot===false||strlen((string)$snapshot)>4000000)ns_rep_out(['ok'=>false,'error'=>'El reporte es demasiado grande para guardarlo.'],422);
...
function ns_rep_email_html(array $items,$intro='',array &$inlineImages=[],$reportTitle='Reporte operativo'){
    $html='<div style="font-family:Arial,sans-serif;color:#183746"><h1 style="color:#1a596b">CLEAR · '.ns_rep_html($reportTitle).'</h1>';
    if($intro!=='')$html.='<p>'.nl2br(ns_rep_html($intro)).'</p>';
    $gridItems=[];
    foreach($items as $row){
      $payload=json_decode((string)ns_value($row,'PAYLOAD_JSON'),true);if(!is_array($payload))$payload=[];
      if(strtolower((string)($payload['kind']??'row'))!=='chart'){$gridItems[]=$row;continue;}
      $html.='<section style="margin:18px 0"><h2 style="font-size:17px;border-bottom:2px solid #1a596b;padding-bottom:5px">'.ns_rep_html(ns_value($row,'TITULO')).'</h2>';
      $path=ns_rep_image_path($payload['image_ref']??'');
      if($path!==''){$cid='clear_chart_'.count($inlineImages).'@clear';$inlineImages[]=['path'=>$path,'cid'=>$cid];$html.='<img src="cid:'.ns_rep_html($cid).'" alt="'.ns_rep_html(ns_value($row,'TITULO')).'" style="display:block;max-width:100%;height:auto;border:1px solid #dbe5e9;border-radius:8px">';}
      else{$fallback=ns_report_grid_sections([$row]);if($fallback){$section=$fallback[0];$html.='<table cellpadding="6" cellspacing="0" style="border-collapse:collapse;width:100%;font-size:11px"><thead><tr>';foreach($section['headers'] as $header)$html.='<th style="text-align:left;background:#1a596b;color:#fff;border:1px solid #c9d7dc">'.ns_rep_html($header).'</th>';$html.='</tr></thead><tbody>';foreach($section['rows'] as $entry){$html.='<tr>';foreach($section['headers'] as $header)$html.='<td style="border:1px solid #d7e0e4">'.ns_rep_html(ns_report_grid_value($entry['values'][$header]??'')).'</td>';$html.='</tr>';}$html.='</tbody></table>';}}
      $html.='<p style="font-size:11px;color:#546a77"><b>Comentario:</b> '.ns_rep_html(trim((string)($payload['context']??''))!==''?(string)$payload['context']:'—').'</p></section>';
    }
    foreach(ns_report_grid_sections($gridItems) as $section){
      $html.='<section style="margin:18px 0"><h2 style="font-size:17px;border-bottom:2px solid #1a596b;padding-bottom:5px">'.ns_rep_html($section['title']).'</h2>';
      $html.='<div style="overflow-x:auto"><table cellpadding="6" cellspacing="0" style="border-collapse:collapse;width:100%;font-size:11px"><thead><tr>';
      foreach($section['headers'] as $header)$html.='<th style="text-align:left;background:#1a596b;color:#fff;border:1px solid #c9d7dc;white-space:nowrap">'.ns_rep_html($header).'</th>';
      $html.='</tr></thead><tbody>';
      foreach($section['rows'] as $entry){$html.='<tr>';foreach($section['headers'] as $header)$html.='<td style="border:1px solid #d7e0e4;vertical-align:top">'.ns_rep_html(ns_report_grid_value($entry['values'][$header]??'')).'</td>';$html.='</tr>';}
...
if($action==='email'){
    ...
    $items=ns_report_items($db,$user);if(!$items)ns_rep_out(['ok'=>false,'error'=>'El reporte no tiene elementos seleccionados.'],422);$titleColumn=ns_report_title_column_for_items($input['title_column']??'',$items);$reportTitle=ns_report_title($titleColumn);$subject=ns_clean($input['subject']??'CLEAR · '.$reportTitle,250);$body=(string)($input['body']??'');$smtp=report_smtp_config();if(!$smtp['host']||!$smtp['from'])ns_rep_out(['ok'=>false,'error'=>'La configuración SMTP está incompleta en Reportes por correo.'],503);
    try{$inline=[];$html=ns_rep_email_html($items,$body,$inline,$reportTitle);$client=new ClearSmtpClient();$client->send($smtp,$to,[],[],$subject,$html,[],$inline);}catch(Throwable $e){ns_rep_out(['ok'=>false,'error'=>'No se pudo enviar el correo: '.$e->getMessage()],500);}
