...
function permissions_get_user($usuario)
{
    ...
    // Las gestiones de telemetría se derivan de la grilla general: quien la ve, accede a ambas pantallas.
    if (in_array('telemetria_general',$menus,true)) {
        foreach(['sin_telemetria_zafiro','gestion_telemetria'] as $k) if(!in_array($k,$menus,true))$menus[]=$k;
    }
...
function permissions_can_menu($key,$usuario=null)
{
    ...
    // Mis Reportes es una función personal disponible para todo usuario activo.
