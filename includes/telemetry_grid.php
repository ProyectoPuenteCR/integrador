...
$zafiroSources=[
  [
    ...
    'active'=>'Resumen de Producción Teórica de Pozo>>Cambio de estado>>Estado_isActive',
...
$zafiroActiveColumn='Zafiro Activo';
$zafiroColumns=[$zafiroStateColumn,$zafiroMethodColumn,$zafiroActiveColumn];
...
function tg_zafiro_active_label($value){
  if($value===null)return '';
  if(is_bool($value))return $value?'true':'false';
  $text=tg_text($value);
  if($text==='')return '';
  $upper=strtoupper($text);
  if(in_array($upper,['1','TRUE','SI','SÍ','YES','ACTIVO','ACTIVE'],true))return 'true';
  if(in_array($upper,['0','FALSE','NO','INACTIVO','INACTIVE'],true))return 'false';
  return '';
}
...
foreach($serverSelectFilters as $filterColumn=>$filterConfig){
  ...
  /*
   * Algunas fuentes guardan en una misma columna conceptos distintos a los
   * que necesita mostrar la pantalla. La grilla general, por ejemplo, recibe
   * BM/PCP/BES desde SCADA y TECCS/TECSS desde TECSS. El mapa se aplica luego
   * de consultar SQL para que filtros, tabla, Excel y reportes compartan el
   * mismo valor de presentación.
   */
  $displayValueMap=[];
  foreach(($filterConfig['display_value_map'] ?? []) as $sourceValue=>$displayValue){
    $sourceValue=strtoupper(trim((string)$sourceValue));
    $displayValue=strtoupper(trim((string)$displayValue));
    if($sourceValue!==''&&$displayValue!=='')$displayValueMap[$sourceValue]=$displayValue;
  }
  $displayDefaultValue=strtoupper(trim((string)($filterConfig['display_default_value'] ?? '')));
  if($displayValueMap||$displayDefaultValue!==''){
    $serverDisplayNormalizers[$filterColumn]=[
      'map'=>$displayValueMap,
  ...
  if($selected!=='TODOS'){
    $sourceExpression='UPPER(LTRIM(RTRIM(CONVERT(nvarchar(100),'.tg_q($filterColumn).'))))';
    $sourceValueGroup=$filterConfig['source_value_groups'][$selected] ?? null;
    $includeValues=[];
    $excludeValues=[];
    if(is_array($sourceValueGroup)){
      $includeValues=array_values(array_unique(array_filter(array_map(static function($value){
        return strtoupper(trim((string)$value));
      },$sourceValueGroup['include'] ?? []),static function($value){return $value!=='';})));
      $excludeValues=array_values(array_unique(array_filter(array_map(static function($value){
        return strtoupper(trim((string)$value));
      },$sourceValueGroup['exclude'] ?? []),static function($value){return $value!=='';})));
    }

    if($includeValues){
      $whereParts[]=$sourceExpression.' IN ('.implode(',',array_fill(0,count($includeValues),'?')).')';
      foreach($includeValues as $includeValue)$whereParams[]=$includeValue;
    }elseif($excludeValues){
      /* Todo valor que no sea TECSS pertenece al canal SCADA. */
      $whereParts[]='('.$sourceExpression.' NOT IN ('.implode(',',array_fill(0,count($excludeValues),'?')).') OR '.tg_q($filterColumn).' IS NULL)';
      foreach($excludeValues as $excludeValue)$whereParams[]=$excludeValue;
    }else{
      $whereParts[]=$sourceExpression.' = ?';
...
if($rows&&$serverDisplayNormalizers){
  foreach($rows as &$serverDisplayRow){
    foreach($serverDisplayNormalizers as $serverDisplayColumn=>$serverDisplayNormalizer){
      $serverDisplaySource=strtoupper(tg_text($serverDisplayRow[$serverDisplayColumn]??''));
      $serverDisplayValue=$serverDisplayNormalizer['map'][$serverDisplaySource]
        ??$serverDisplayNormalizer['default'];
      if($serverDisplayValue!=='')$serverDisplayRow[$serverDisplayColumn]=$serverDisplayValue;
    }
  }
  unset($serverDisplayRow);
}
...
if($zafiroEnabled){
  ...
  if($db->ok()){
    ...
    foreach($zafiroSources as $zafiroSource){
      ...
      $zafiroSourceActive=trim((string)($zafiroSource['active']??''));

      if($zafiroSourceWell===''||$zafiroSourceState===''||$zafiroSourceMethod===''||$zafiroSourceActive===''){
        ...
        foreach($zafiroMetadata as $zafiroMeta){
          ...
          if($zafiroSourceActive===''&&(
            in_array($zafiroSourceKey,['ACTIVO','ISACTIVE','ZAFIROACTIVO','ACTIVOZAFIRO','ESTADOISACTIVE'],true)
            || strpos($zafiroSourceKey,'ZAFIROACTIVO')!==false
            || strpos($zafiroSourceKey,'ACTIVOZAFIRO')!==false
            || strpos($zafiroSourceKey,'ESTADOISACTIVE')!==false
          )){
            $zafiroSourceActive=$zafiroSourceName;
          }
          if($zafiroSourceState===''&&(
            ...
            || (strpos($zafiroSourceKey,'CAMBIODEESTADOESTADO')!==false&&strpos($zafiroSourceKey,'ISACTIVE')===false)
          )){
      ...
      /*
       * Zafiro Activo se consulta por separado. De esta forma, un nombre de
       * campo ausente o un problema puntual en isActive nunca puede anular
       * Estado Zafiro y Método Zafiro, cuya consulta queda idéntica a la
       * versión validada del 14/09/2026.
      */
      if($zafiroSourceActive!==''){
        $zafiroActiveSql='SELECT '
          .tg_q($zafiroSourceWell).' AS [__ZAFIRO_POZO],'
          .'CONVERT(nvarchar(10),'.tg_q($zafiroSourceActive).') AS [__ZAFIRO_ACTIVO]'
          .' FROM '.$zafiroObjectSql
          .' WHERE '.tg_q($zafiroSourceWell).' IS NOT NULL';
        if($zafiroOrderParts)$zafiroActiveSql.=' ORDER BY '.implode(',',$zafiroOrderParts);
        $zafiroActiveRows=$db->all($zafiroActiveSql);
      }
      break;
    }
    ...
    foreach($zafiroActiveRows as $zafiroActiveRow){
      $zafiroActiveValue=tg_zafiro_active_label($zafiroActiveRow['__ZAFIRO_ACTIVO']??null);
      if($zafiroActiveValue==='')continue;
      foreach(tg_zafiro_well_keys($zafiroActiveRow['__ZAFIRO_POZO']??'') as $zafiroKey){
        if(!isset($zafiroMap[$zafiroKey])){
          $zafiroMap[$zafiroKey]=[
            $zafiroStateColumn=>'',
            $zafiroMethodColumn=>'',
            $zafiroActiveColumn=>$zafiroActiveValue
          ];
        }elseif(($zafiroMap[$zafiroKey][$zafiroActiveColumn]??'')===''){
          $zafiroMap[$zafiroKey][$zafiroActiveColumn]=$zafiroActiveValue;
        }
      }
    }
  }
...
if($zafiroEnabled){
  /* Estado Zafiro, Método Zafiro y Zafiro Activo quedan juntos después del estado operativo. */
...
if($zafiroActiveCounts)$filterGroups[]=['column'=>$zafiroActiveColumn,'label'=>$zafiroActiveColumn,'values'=>$zafiroActiveCounts];
