(function(root){
  ...
  function refresh(){
    ...
    doc.getElementById('pfRowCount').textContent=num.format(data.length)+' de '+num.format(rows.length)+(cfg.manualParts?' partes en la semana. Los mismos filtros se aplican a la evolución.':' pozos en la lectura. Los mismos filtros se aplican a la evolución.');
    ...
    doc.getElementById('pfTrendMessage').textContent=!cfg.historyReady?'Histórico no disponible. No se muestran valores estimados.':(!saved.size?(cfg.manualParts?'Todavía no hay partes semanales guardados.':'Todavía no hay lecturas semanales guardadas.'):saved.size+(cfg.manualParts?' semanas con partes. Los huecos indican semanas sin carga; los totales corresponden solo a los pozos informados.':' semanas con registros. Los huecos indican semanas sin captura; la lectura actual no se agrega hasta guardarla.'));
  ...
  Array.from(doc.querySelectorAll('[data-pf-add-chart]')).forEach(function(button){button.addEventListener('click',function(){
    ...
    var payload=Object.assign({},item.payload,{context:(item.id==='trend'?(cfg.manualParts?'Partes manuales por semana; solo pozos informados. ':'Última lectura guardada por semana (no cierre automático). '):cfg.label+'. ')+'Captura de pantalla: '+cfg.stamp+'. Filtros: '+summary+(item.zone?'. Zona: '+item.zone:'')});
