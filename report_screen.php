...
if($screen==='comentarios_semana'){
  ...
  $cards=[['Comentarios 7 días',$result['total']],['Alarmas',$bySource['alarmas']??0],['Novedades',$bySource['novedades']??0],['Instalaciones / pozos',($bySource['instalaciones']??0)+($bySource['pozos']??0)],['Usuarios',count($users)]];$subtitle='Todos los comentarios · '.$from.' al '.$to;
}elseif($screen==='analisis_ia'){
...
<?php if(isset($data)&&$screen==='comentarios_semana'):?><section class="panel"><h2>Comentarios de la última semana</h2><table><thead><tr><th>Actualizado</th><th>Origen</th><th>Asunto</th><th>Contexto</th><th>Comentario</th><th>Usuario</th><th>Período</th></tr></thead><tbody><?php foreach($data as $r):?><tr><td><?=rh($r['FECHA_ACTUALIZACION']??'')?></td><td><?=rh($sources[$r['ORIGEN']??'']??($r['ORIGEN']??''))?></td><td><?=rh($r['ASUNTO']??'')?></td><td><?=rh($r['CONTEXTO']??'')?></td><td><?=rh($r['COMENTARIO']??'')?></td><td><?=rh($r['USUARIO']??'')?></td><td><?=rh(($r['FECHA_DESDE']??'').' - '.($r['FECHA_HASTA']??''))?></td></tr><?php endforeach;?><?php if(!$data):?><tr><td colspan="7">No hay comentarios en los últimos 7 días.</td></tr><?php endif;?></tbody></table></section><?php endif;?>
