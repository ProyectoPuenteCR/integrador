...
function permissions_profiles()
{
    ...
    return [
        ...
        'supervisor' => ['label'=>'Supervisor','comments'=>[1,1,1,1,0],'menus'=>array_values(array_diff($allMenus,['admin_usuarios','admin_supervisores_instalaciones','config_pi','config_menu','audit_log','versiones','reportes','tags_filtrados']))],
        'operador' => ['label'=>'Operador','comments'=>[1,1,1,0,0],'menus'=>['dashboard','dashboard_inst_sup','dashboard_pozos','pozos_por_baterias','telemetria_general','monitoreo_pozos','telemetria_pcp','telemetria_bes','telemetria_tecss','tecss_3sigma','inyeccion_agua','micros_contables','micros_contables_cierre','pi_historico','alarmas24h','pozos_alarmas24','pozos_todas_alarmas','instalaciones_alarmas_semanal','novedades_semanales_panel','novedades_semanales_malos_actores','novedades_semanales_comparativa','novedades_semanales_seguimiento','novedades_semanales_reporte','reportes_guardados','alarmas_activas','suprimidas','reconocidas','reconocidas_usr','top20_all','top20_24h','ranking24h','todas_alarmas','pozos','pozos_tecss','importadas','analisis_ia']],
        'consulta' => ['label'=>'Consulta','comments'=>[1,0,0,0,0],'menus'=>['dashboard','dashboard_inst_sup','dashboard_pozos','pozos_por_baterias','telemetria_general','monitoreo_pozos','telemetria_pcp','telemetria_bes','telemetria_tecss','tecss_3sigma','inyeccion_agua','micros_contables','micros_contables_cierre','pi_historico','alarmas24h','pozos_alarmas24','pozos_todas_alarmas','instalaciones_alarmas_semanal','novedades_semanales_panel','novedades_semanales_malos_actores','novedades_semanales_comparativa','novedades_semanales_seguimiento','novedades_semanales_reporte','reportes_guardados','alarmas_activas','suprimidas','reconocidas','reconocidas_usr','top20_all','top20_24h','ranking24h','todas_alarmas','pozos','pozos_tecss','importadas','analisis_ia']],
        'pi_mantenimiento' => ['label'=>'PI / Mantenimiento','comments'=>[1,1,1,0,0],'menus'=>['dashboard','dashboard_inst_sup','dashboard_pozos','pozos_por_baterias','telemetria_general','monitoreo_pozos','telemetria_pcp','telemetria_bes','telemetria_tecss','tecss_3sigma','inyeccion_agua','micros_contables','micros_contables_cierre','pi_historico','alarmas24h','pozos_alarmas24','pozos_todas_alarmas','instalaciones_alarmas_semanal','novedades_semanales_panel','novedades_semanales_malos_actores','novedades_semanales_comparativa','novedades_semanales_seguimiento','novedades_semanales_reporte','reportes_guardados','alarmas_activas','top20_all','top20_24h','ranking24h','todas_alarmas','analisis_ia']],
    ];
}
...
function permissions_get_user($usuario)
{
    ...
    if (in_array('pozos',$menus,true) || in_array('pozos_tecss',$menus,true)) { foreach(['telemetria_general','monitoreo_pozos','telemetria_pcp','telemetria_bes','telemetria_tecss','tecss_3sigma','inyeccion_agua'] as $k) if(!in_array($k,$menus,true))$menus[]=$k; }
    if (in_array('telemetria_tecss',$menus,true) && !in_array('tecss_3sigma',$menus,true)) $menus[]='tecss_3sigma';
    ...
    if (array_intersect(['pozos_por_baterias','monitoreo_pozos','telemetria_general','alarmas24h'],$menus)) {
        foreach(['pozos_alarmas24','pozos_todas_alarmas'] as $k) if(!in_array($k,$menus,true))$menus[]=$k;
    }
    ...
    if(strtolower((string)($r['PERFIL']??''))==='admin'){
        ...
        foreach(['micros_contables','micros_contables_cierre','versiones','reportes','tags_filtrados','admin_supervisores_instalaciones'] as $k){
            if(!in_array($k,$menus,true))$menus[]=$k;
        }
    }
...
function permissions_can_menu($key,$usuario=null)
{
    ...
    if($key==='novedades_semanales_monitoreo') return in_array('novedades_semanales_panel',$p['menus'],true) && in_array('monitoreo_pozos',$p['menus'],true);
